...
const providerOptions = [
  { label: "Fake 测试 Provider", value: "fake" },
...
const profileLayerOptions = [
  { label: "AI识别补全", value: "basic_extraction_profile" },
  { label: "AI联网补全", value: "web_enrichment_profile" }
];
...
const aiSettingsHint =
  "AI Profile 是 AI 能力层的底座配置。第一层和第二层可以绑定不同模型，后续漏洞补全不会向 AI 发送资产清单或匹配结果。";

const defaultFormValues: AIProfileFormValues = {
  profile_key: "basic_extraction_profile",
  display_name:
    profileLayerDescriptionMap.get("basic_extraction_profile") ?? "源信息快速提取",
  provider: "fake",
  model_vendor: "openai",
  base_url: null,
  api_key: null,
...
function profileLayerLabel(profileKey: string) {
  return profileLayerLabelMap.get(profileKey) ?? "未绑定能力层";
}
...
export default function AiSettingsPage() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      messageApi.success("AI Profile 已保存");
    },
    onError: (error) => {
      messageApi.error(error instanceof Error ? error.message : "保存 AI Profile 失败");
    }
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      messageApi.success("AI Profile 已删除");
    },
    onError: (error) => {
      messageApi.error(error instanceof Error ? error.message : "删除 AI Profile 失败");
    }
  });
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (result) => {
      setTestResult({
        ...
        text: result.success
          ? `连接测试成功，模型 ${result.model}，耗时 ${result.latency_ms ?? 0} ms`
          : `连接测试失败：${result.error_message || result.status}`
      });
      if (result.success) {
        messageApi.success("AI Profile 连接测试成功");
      } else {
        messageApi.error("AI Profile 连接测试失败");
      }
    },
    onError: (error) => {
      messageApi.error(error instanceof Error ? error.message : "连接测试失败");
    }
  });
  ...
  const automationMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      messageApi.success("AI 自动化设置已保存");
    },
    onError: (error) => {
      messageApi.error(error instanceof Error ? error.message : "保存 AI 自动化设置失败");
    }
  });
  ...
  const forceWebBatchMutation = useMutation({
    ...
    onSuccess: (result) => {
      messageApi.success(`强制联网补全任务已创建，选中 ${result.selected_count} 条`);
    ...
    onError: (error) => {
      messageApi.error(error instanceof Error ? error.message : "强制联网补全任务创建失败");
    }
  });
  ...
  const columns: ColumnsType<AIProfile> = [
    ...
    },
    {
      ...
      render: (_, profile) => (
        <Space wrap size={4}>
          {profile.enabled ? <Tag color="green">启用</Tag> : <Tag>停用</Tag>}
          {profile.json_mode && <Tag color="blue">JSON</Tag>}
          {profile.supports_web_search && <Tag color="purple">联网</Tag>}
        </Space>
      )
    },
    {
      ...
      <div className="ai-ops-grid">
        ...
        >
          ...
          >
            <div className="ai-automation-switches">
              <Form.Item label="AI 总开关" name="ai_enabled" valuePropName="checked">
            ...
            <div className="form-inline-grid">
              <Form.Item label="自动采纳策略" name="ai_auto_accept_policy">
                <Select
                  options={[
                    { label: "严格模式（字段证据完整）", value: "strict" },
                    { label: "适中模式（产品与版本）", value: "moderate" },
                    { label: "宽松模式（仅产品即可匹配）", value: "relaxed" }
                  ]}
                />
              </Form.Item>
              <Form.Item label="自动采纳阈值" name="ai_auto_accept_confidence">
                <InputNumber min={0} max={1} step={0.01} style={{ width: "100%" }} />
              </Form.Item>
              <Form.Item label="联网自动采纳阈值" name="ai_web_auto_accept_confidence">
                <InputNumber min={0} max={1} step={0.01} style={{ width: "100%" }} />
              </Form.Item>
              <Form.Item label="二层日上限" name="ai_layer2_daily_limit">
                <InputNumber min={1} max={10000} style={{ width: "100%" }} />
              </Form.Item>
              <Form.Item label="批量最大数" name="ai_batch_max_size">
            ...
            <Space className="ai-settings-actions" wrap>
              ...
              <Popconfirm
                title="强制重新联网补全"
                description="会跳过已有 AI 补全缓存，对当前待补全漏洞重新发起联网补全，可能消耗较多 Token。"
                okText="开始"
                cancelText="取消"
                okButtonProps={{
                  danger: true,
        ...
        >
          ...
          <Row gutter={[12, 12]} className="ai-stats-grid">
            <Col xs={12} md={6}>
              <Statistic title="调用" value={stats?.today_call_count ?? 0} />
            </Col>
            <Col xs={12} md={6}>
              <Statistic title="Token" value={stats?.today_token_count ?? 0} />
            </Col>
            <Col xs={12} md={6}>
              <Statistic title="一层成功率" value={formatRate(stats?.layer1_success_rate)} />
            </Col>
            <Col xs={12} md={6}>
              <Statistic title="二层成功率" value={formatRate(stats?.layer2_success_rate)} />
            </Col>
          </Row>
          <Space className="ai-status-strip" wrap size={8}>
            <Tag color="blue" icon={<CircleAlert size={13} />}>
              待复核 {stats?.pending_review_count ?? 0}
            </Tag>
            <Tag color="green" icon={<CheckCircle2 size={13} />}>
              已采纳 {stats?.accepted_count ?? 0}
            </Tag>
            <Tag color="cyan" icon={<CheckCircle2 size={13} />}>
              自动采纳 {stats?.auto_accepted_count ?? 0}
            </Tag>
            <Tag color="red">失败 {stats?.failed_count ?? 0}</Tag>
            <Tag>不足 {stats?.insufficient_count ?? 0}</Tag>
          </Space>
          ...
          >
            <div className="profile-settings-section">
              ...
              <div className="profile-settings-main-grid">
                <Form.Item
                  label="配置能力层"
                  name="profile_key"
                  rules={[{ required: true, message: "请选择配置能力层" }]}
                >
                  <Select
            ...
            <div className="profile-settings-section">
              ...
              <div className="profile-settings-main-grid">
                <Form.Item
                  ...
                  label="Base URL"
                  name="base_url"
                  tooltip="OpenAI Compatible Provider 使用，例如 https://api.example.com/v1"
                >
                  <Input placeholder="仅 OpenAI Compatible 需要" />
                </Form.Item>

                <Form.Item
                  className="profile-field-wide"
                  label={selectedProfile.has_api_key ? "API Key（留空则保持不变）" : "API Key"}
                  name="api_key"
                >
            ...
            <div className="profile-settings-section">
              ...
              <div className="profile-settings-limit-grid">
                ...
                <Form.Item label="输出上限" name="max_tokens">
                  <InputNumber min={1} max={100000} placeholder="不限" />
                </Form.Item>
                <Form.Item label="温度" name="temperature">
                  <InputNumber min={0} max={2} step={0.1} />
                </Form.Item>
                <Form.Item label="日调用上限" name="daily_call_limit">
                  <InputNumber min={1} placeholder="不限" />
                </Form.Item>
                <Form.Item label="日 Token 上限" name="daily_token_limit">
                  <InputNumber min={1} placeholder="不限" />
                </Form.Item>
              </div>
            </div>
            ...
            <div className="profile-settings-section">
              <Space className="profile-prompt-title-row" size={8} wrap>
                ...
                {selectedPromptTemplate?.customized ? <Tag color="blue">已自定义</Tag> : <Tag>默认模板</Tag>}
              </Space>
              {selectedPromptTemplate ? (
                <div className="profile-prompt-edit-grid">
                  ...
                  <Form.Item
                    ...
                    rules={promptTemplateRules("User Prompt 模板")}
                    tooltip="可使用 {output_contract} 和 {enrichment_input_json} 占位符"
                  >
                  ...
                  <Form.Item
                    ...
                    rules={promptTemplateRules("输出契约")}
                  >
                    <Input.TextArea className="prompt-edit-textarea" rows={8} />
                  </Form.Item>
                </div>
                ...
                <Alert
                  type="info"
                  showIcon
                  message="当前 Profile 暂未绑定平台内置提示词模板"
                />
              )}
            </div>
            ...
            <div className="profile-settings-control-row">
              <div className="profile-settings-switch-panel">
                ...
                <Space className="profile-settings-switch-row" wrap size={18}>
                  ...
                  <Form.Item name="json_mode" valuePropName="checked">
                    <Switch checkedChildren="JSON" unCheckedChildren="普通" />
                  </Form.Item>
                  <Form.Item name="supports_web_search" valuePropName="checked">
                    <Switch checkedChildren="支持联网" unCheckedChildren="无联网" />
                  </Form.Item>
                  <Form.Item name="allow_external_network" valuePropName="checked">
                    <Switch checkedChildren="允许外联" unCheckedChildren="禁止外联" />
                  </Form.Item>
                </Space>
              </div>
              ...
              <Space wrap className="profile-action-row">
                ...
                <Popconfirm
                  title="删除 AI Profile"
                  description="删除后会保留历史调用和补全记录，但这条 Profile 配置会被移除。"
                  okText="确认删除"
                  cancelText="取消"
