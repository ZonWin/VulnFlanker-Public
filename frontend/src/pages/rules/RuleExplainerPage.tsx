import { t, useI18n } from "@/app/i18n";
import {
  Alert,
  Button,
  Card,
...
const matchingRules: MatchingRule[] = [
  {
    ...
    icon: PackageSearch,
    color: "blue",
    summaryKey: "先确认资产组件、服务或系统信息中，是否出现漏洞影响的产品或常见别名。",
    rows: [
      {
        key: "product-missing",
        scenario: t("漏洞缺少产品字段"),
  ...
  },
  {
    ...
    icon: FileSearch,
    color: "cyan",
    summaryKey: "在产品命中后比较资产版本、影响版本和修复版本，判断是否落入影响范围。",
    rows: [
      {
      ...
      },
      {
        ...
        scenario: t("影响文本精确点名资产 OS/Kernel 版本"),
        status: "affected",
  ...
  },
  {
    ...
    icon: Layers3,
    color: "green",
    summaryKey: "当漏洞声明受影响 OS 时，检查资产平台、系统族和系统版本是否命中。",
    rows: [
      {
        key: "os-none",
        scenario: t("漏洞未定义 OS 条件"),
  ...
  },
  {
    ...
    icon: Puzzle,
    color: "gold",
    summaryKey: "检查资产证据中是否出现漏洞要求的模块或功能开关，例如特定插件、协议或配置。",
    rows: [
      {
        key: "feature-none",
        scenario: t("未定义模块/功能条件"),
        status: "not_applicable",
        confidenceKey: "not_applicable",
        defaultConfidence: 0
      },
  ...
  },
  {
    ...
    icon: Network,
    color: "purple",
    summaryKey: "当漏洞需要公网可达时，检查匹配服务是否真的暴露在公网。",
    rows: [
      {
        key: "exposure-none",
        scenario: t("漏洞不要求公网访问"),
        status: "not_applicable",
        confidenceKey: "not_applicable",
        defaultConfidence: 0
      },
...
const priorityThresholdRows: EditableFlatRow[] = [
  { key: "low", field: "low", label: t("low 起始阈值"), codePath: "backend/app/services/risk.py" },
  { key: "medium", field: "medium", label: t("medium 起始阈值"), codePath: "backend/app/services/risk.py" },
  { key: "high", field: "high", label: t("high 起始阈值"), codePath: "backend/app/services/risk.py" },
  {
    key: "critical",
    field: "critical",
    label: t("critical 起始阈值"),
    codePath: "backend/app/services/risk.py"
  }
];
  ...
  return [
    {
      ...
      factor: "severity",
      source: t("漏洞 CVSS"),
      value: t("直接使用 CVSS，缺失则为 0"),
      weight: formatNumber(flatValue(weights, defaultRiskWeights, "severity"))
    },
    {
      key: "exploitability",
      factor: "exploitability",
      source: t("KEV / PoC / 野外利用 / EPSS"),
      value: t("取最大信号：KEV={{v0}}，PoC={{v1}}，野外利用={{v2}}，EPSS=epss*{{v3}}", { v0: nestedValue(values, defaultRiskFactorValues, "exploitability", "kev"), v1: nestedValue(values, defaultRiskFactorValues, "exploitability", "poc"), v2: nestedValue(values, defaultRiskFactorValues, "exploitability", "wild_exploitation"), v3: nestedValue(values, defaultRiskFactorValues, "exploitability", "epss_multiplier") }),
    ...
    },
    {
      ...
      factor: "exposure",
      source: t("资产暴露面"),
      value: t("公网暴露={{v0}}，internet/public/external={{v1}}，dmz={{v2}}，其他={{v3}}", { v0: nestedValue(values, defaultRiskFactorValues, "exposure", "public_exposure"), v1: nestedValue(values, defaultRiskFactorValues, "exposure", "internet"), v2: nestedValue(values, defaultRiskFactorValues, "exposure", "dmz"), v3: nestedValue(values, defaultRiskFactorValues, "exposure", "default") }),
    ...
    },
    {
      ...
      source: t("资产 last_seen_at"),
      value: t("24 小时内={{v0}}，7 天内={{v1}}，超过 7 天={{v2}}，无时间戳={{v3}}", { v0: nestedValue(values, defaultRiskFactorValues, "asset_freshness", "fresh"), v1: nestedValue(values, defaultRiskFactorValues, "asset_freshness", "stale"), v2: nestedValue(values, defaultRiskFactorValues, "asset_freshness", "critical"), v3: nestedValue(values, defaultRiskFactorValues, "asset_freshness", "unknown") }),
...
function RuleCard({
  ...
  return (
    ...
    >
      ...
      <Typography.Paragraph className="rule-summary">
        {t(rule.summaryKey)}
...
export default function RuleExplainerPage() {
  ...
  const activeWarnings = useMemo(() => {
    ...
    if (Math.abs(activeWeightTotal - 1) > 0.001) {
      warnings.unshift(
        t("当前草稿权重合计 {{v0}}，建议保持 1.0000", { v0: activeWeightTotal.toFixed(4) })
      );
    }
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      messageApi.success(t("规则数值配置已保存"));
    },
    onError: (error) => {
      messageApi.error(error instanceof Error ? error.message : t("规则数值配置保存失败"));
    }
  });
  ...
  const resetMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      void queryClient.invalidateQueries({ queryKey: ["rule-config"] });
      void queryClient.invalidateQueries({ queryKey: ["match-results", "risk-config"] });
      messageApi.success(t("已恢复默认规则数值"));
    },
    onError: (error) => {
      messageApi.error(error instanceof Error ? error.message : t("恢复默认配置失败"));
    }
  });
  ...
  function startEditing() {
    Modal.confirm({
      title: t("确认编辑规则数值？"),
      content:
        t("这些数值会影响匹配置信度、风险因子分值、风险评分权重和风险优先级映射，可能改变整体风险分析结果。请确认已理解影响范围后再继续操作。"),
      okText: t("继续编辑"),
      cancelText: t("取消"),
  ...
  return (
    <Space className="page-stack rule-explainer-page" orientation="vertical" size={16}>
      ...
      <Alert
        ...
        showIcon
        message={t("一句话理解")}
        description={t("先用规则链判断资产是否受漏洞影响并给出置信度，再结合漏洞严重度、可利用性、资产暴露、重要性、验证状态和资产新鲜度等 7 个因子，生成 0 到 10 的风险分和优先级。")}
      />

      <Row gutter={[16, 16]}>
        <Col xs={24} xl={12}>
          <Card className="content-card explain-band" title={t("阶段一：匹配规则流水线")}>
            <div className="flow-strip">
              <div className="flow-node">
                <Database size={24} />
                <strong>{t("数据输入")}</strong>
                <span>{t("资产画像、组件、暴露面、漏洞条件")}</span>
              </div>
              <GitBranch className="flow-arrow" size={22} />
              <div className="flow-node">
                <Boxes size={24} />
                <strong>{t("构建上下文")}</strong>
                <span>{t("生成 MatchContext")}</span>
              </div>
              <GitBranch className="flow-arrow" size={22} />
              <div className="flow-node">
                <Filter size={24} />
                <strong>{t("规则评估")}</strong>
                <span>{t("5 条规则依次输出结论")}</span>
              </div>
              <GitBranch className="flow-arrow" size={22} />
              <div className="flow-node">
                <CheckCircle2 size={24} />
                <strong>{t("结果聚合")}</strong>
                <span>{t("状态、置信度、原因、证据")}</span>
              </div>
            </div>
          </Card>
        </Col>
        <Col xs={24} xl={12}>
          <Card className="content-card explain-band" title={t("阶段二：风险模型 risk-v2.0")}>
            <div className="flow-strip">
              <div className="flow-node">
                <Sparkles size={24} />
                <strong>{t("7 个因子")}</strong>
                <span>{t("漏洞、资产、验证多维信号")}</span>
              </div>
              <GitBranch className="flow-arrow" size={22} />
              <div className="flow-node">
                <Calculator size={24} />
                <strong>{t("加权计算")}</strong>
                <span>{t("Σ 因子分 * 权重")}</span>
              </div>
              <GitBranch className="flow-arrow" size={22} />
              <div className="flow-node">
                <Flag size={24} />
                <strong>{t("风险优先级")}</strong>
      ...
      <section id="rules" className="scroll-section">
        <Typography.Title level={2}>{t("匹配规则流水线")}</Typography.Title>
        <Typography.Paragraph className="explanation-text">
          {t("每条规则都会输出状态、置信度、原因和证据。状态不是简单投票，而是按阻断、复核、受影响的优先级合并。")}</Typography.Paragraph>
        <Row gutter={[16, 16]}>
      ...
      <section id="trace" className="scroll-section">
        <Card className="content-card" title={t("Pipeline 如何合并最终状态与置信度")}>
          <Timeline
            items={[
              {
                ...
                children: (
                  <Space direction="vertical" size={4}>
                    <Typography.Text strong>
                      {t("存在阻断型")}<StatusTagInline value="not_affected" />？
                    </Typography.Text>
                    <Typography.Text>
                      {t("最终状态为")}<StatusTagInline value="not_affected" />{t("，置信度取阻断结果中的最大值。")}</Typography.Text>
                  </Space>
                )
              },
              {
                ...
                children: (
                  <Space direction="vertical" size={4}>
                    <Typography.Text strong>
                      {t("否则，存在")}<StatusTagInline value="needs_review" />？
                    </Typography.Text>
                    <Typography.Text>
                      {t("最终状态为")}<StatusTagInline value="needs_review" />{t("，置信度取 review 结果中的最小值。")}</Typography.Text>
                  </Space>
                )
              },
              {
                ...
                children: (
                  <Space direction="vertical" size={4}>
                    <Typography.Text strong>
                      {t("否则，存在")}<StatusTagInline value="affected" />？
                    </Typography.Text>
                    <Typography.Text>
                      {t("最终状态为")}<StatusTagInline value="affected" />{t("，置信度取 affected 结果中的最小值。")}</Typography.Text>
                  </Space>
                )
              },
              {
                ...
                children: (
                  <Space direction="vertical" size={4}>
                    <Typography.Text strong>{t("没有有效结论")}</Typography.Text>
                    <Typography.Text>
                      {t("默认进入")}<StatusTagInline value="needs_review" />{t("，置信度为 0.30。")}</Typography.Text>
                  </Space>
                )
              }
            ]}
      ...
      <section id="risk-factors" className="scroll-section">
        <Card className="content-card" title={t("风险模型 risk-v2.0 与 7 个风险因子")}>
          <Space className="risk-formula" direction="vertical" size={10}>
            <Typography.Text strong>
              {t("仅当匹配状态不是")}<StatusTagInline value="not_affected" /> {t("时进入风险计算；否则风险分直接为 0。")}</Typography.Text>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={12}>
          <Card className="content-card" title={t("风险优先级映射")}>
            <Descriptions
              bordered
              size="small"
              column={1}
        ...
        <Col xs={24} lg={12}>
          <Card className="content-card" title={t("示例计算")}>
            <Descriptions
              bordered
              size="small"
              column={1}
              items={[
                {
                  key: "input",
                  label: t("输入"),
                  children: t("affected、CVSS 6.0、KEV、公网暴露、high 资产、置信度 0.76、未验证、快照超过 7 天")
                },
                {
                  key: "score",
                  label: t("风险分"),
                ...
                },
                {
                  key: "fresh",
                  label: t("快照新鲜时"),
                  children: t("asset_freshness 从 2 提升到 10，总分约 7.35，优先级变为 high")
                }
              ]}
            />
          </Card>
      ...
      <section id="data-sources" className="scroll-section">
        <Typography.Title level={2}>{t("关键数据来源")}</Typography.Title>
        <Row gutter={[16, 16]}>
          ...
          extra={
            <Space wrap>
              <Typography.Text type="secondary">
                {t("权重合计")}{formatNumber(activeWeightTotal, 4)}
              </Typography.Text>
              {!isEditing ? (
                ...
                <>
                  ...
                  >
                    {t("保存配置")}</Button>
                  <Button onClick={cancelEditing}>{t("取消编辑")}</Button>
                  <Popconfirm
                    title={t("恢复默认数值？")}
                    okText={t("恢复")}
                    cancelText={t("取消")}
        ...
        >
          <Space direction="vertical" size={16} className="full-width">
            ...
            <Alert
              ...
              message={
                isEditing
                  ? t("当前处于编辑模式。数值调整可能影响整体风险分析，请谨慎保存。")
                  : t("数值配置当前为只读状态。点击编辑并确认风险提示后，才能修改置信度赋值、风险因子分值、风险权重和优先级阈值。")
              }
              description={t("保存后对新评估和重评估生效，已存风险快照需要重评估刷新；规则判断逻辑仍由对应代码文件控制。")}
            />
            <Typography.Title level={4}>{t("流水线置信度赋值")}</Typography.Title>
            <Table<EditableNestedRow>
              ...
              scroll={{ x: 900 }}
            />
            <Typography.Title level={4}>{t("风险模型因子分值")}</Typography.Title>
            <Table<EditableNestedRow>
              rowKey="key"
              size="small"
              loading={configQuery.isLoading}
              ...
              scroll={{ x: 900 }}
            />
            <Typography.Title level={4}>{t("风险评分权重")}</Typography.Title>
            <Table<EditableFlatRow>
              ...
              scroll={{ x: 760 }}
            />
            <Typography.Title level={4}>{t("风险优先级映射")}</Typography.Title>
            <Table<EditableFlatRow>
              rowKey="key"
              size="small"
              loading={configQuery.isLoading}
